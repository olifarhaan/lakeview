...
def setup_user(data={}):
    ...
    user_token = authenticate_and_retrieve_token(
        retrieved_data["email"], user_data["password"]
    )
...
def setup_floor(user_token, data={}):
    floor_data = {"name": f"{get_unique_id('Floor')}"}
    floor_data.update(data)
    response = post_request(
        "floors", floor_data, headers=get_headers_with_auth(user_token)
    )
...
def setup_room_class(user_token, data={}):
    ...
    response = post_request(
        "room-classes", room_class_data, headers=get_headers_with_auth(user_token)
    )
    return response.json()["id"], response.json()
...
def setup_room(user_token, data={}):
    ...
    response = post_request(
        "rooms", room_data, headers=get_headers_with_auth(user_token)
    )
    return response.json()["id"], response.json()
...
def setup_rooms(user_token, noOfRooms, roomClassId=None, floorId=None):
    ...
    def attempt_setup_room(i):
        data = {
            "roomClassId": roomClassId,
            "floorId": floorId,
            "roomNumber": f"{get_unique_id('Room')}-{i}",
            "roomStatus": "AVAILABLE",
        }
        response = post_request(
            "rooms", data, headers=get_headers_with_auth(user_token)
        )
    ...
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(attempt_setup_room, range(noOfRooms)))

    return results, roomClassId, floorId
